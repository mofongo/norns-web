...
// Lua bootstrap: sets up the mod hook system and package.preload for 'core/mods'.
// Injected into every Lua state before mods or user scripts run.
const MOD_BOOTSTRAP_LUA = `
_mod_hooks = {}
mod = { hook = {} }

function mod.hook.register(name, id, fn)
  _mod_hooks[name] = _mod_hooks[name] or {}
  _mod_hooks[name][id] = fn
end

function mod.hook.deregister(name, id)
  if _mod_hooks[name] then _mod_hooks[name][id] = nil end
end

function _norns_fire_hook(name)
  local h = _mod_hooks[name]
  if not h then return end
  for id, fn in pairs(h) do
    local ok, err = pcall(fn)
    if not ok then
      print("[mod] hook error [" .. tostring(name) .. "][" .. tostring(id) .. "]: " .. tostring(err))
    end
  end
end

package.preload['core/mods'] = function() return mod end
`;

export async function runLuaScript(luaSrc, canvas, audioCtx, { modSources = [] } = {}) {
  screen.init(canvas);
  ...
  // Helper: execute a Lua source string; throws on parse/runtime error
  function execLua(src, chunkName) {
    const loadSt = luaL_loadstring(L, ls(src));
    if (loadSt !== LUA_OK) {
      const err = lua_tojsstring(L, -1);
      lua_pop(L, 1);
      throw new Error(`[${chunkName}] Lua parse error: ${err}`);
    }
    const execSt = lua_pcall(L, 0, LUA_MULTRET, 0);
    if (execSt !== LUA_OK) {
      const err = lua_tojsstring(L, -1);
      lua_pop(L, 1);
      throw new Error(`[${chunkName}] Lua runtime error: ${err}`);
    }
  }

  // Install the mod system (hooks, package.preload['core/mods'])
  execLua(MOD_BOOTSTRAP_LUA, "mod-bootstrap");
  ...
  // ── engine ──────────────────────────────────────────────────────────────────
  //
  // engine.name = "X"       — declare engine; triggers async SynthDef load
  // engine.someCmd(args)    — any engine command dispatched to engine-bridge
  //
  // Supported engines (Sonic Pi approximations):
  //   PolySub       → sonic-pi-prophet   (subtractive poly, persistent voices)
  //   PolyPerc      → sonic-pi-beep      (percussive, self-releasing triggers)
  //   MollyThePoly  → sonic-pi-prophet   (paraphonic synth, persistent voices)
  //
  // For full-fidelity, compile the engine's .sc file to a binary .scsyndef and
  // place it at engines/{EngineName}.scsyndef — it will be preferred automatically.

  lua_newtable(L);  // engine table (stays empty; all access goes through metatable)

  lua_newtable(L);  // metatable

  // __newindex — intercepts: engine.name = "X"
  lua_pushjsfunction(L, (L) => {
    const key = lua_tojsstring(L, 2);
    if (key === "name") {
      const name = lua_tojsstring(L, 3);
      if (name) {
        engineBridge.setEngine(name).catch(err => {
          console.warn(`[engine] init failed for "${name}":`, err.message ?? err);
        });
      }
    }
    // Don't store anything in the table — engine has no real fields
    return 0;
  });
  lua_setfield(L, -2, ls("__newindex"));

  // __index — returns a callable for any engine command name
  lua_pushjsfunction(L, (L) => {
    const key = lua_tojsstring(L, 2);
    lua_pushjsfunction(L, (innerL) => {
      const nargs = lua_gettop(innerL);
      const args = [];
      for (let i = 1; i <= nargs; i++) {
        const v = lua_tonumber(innerL, i);
        args.push(v != null ? v : lua_tojsstring(innerL, i));
      }
      engineBridge.command(key, args);
      return 0;
    });
    return 1;
  });
  lua_setfield(L, -2, ls("__index"));

  lua_setmetatable(L, -2);
  lua_setglobal(L, ls("engine"));
  ...
  // Helper: fire a named mod hook
  function callHook(hookName) {
    lua_getglobal(L, ls("_norns_fire_hook"));
    if (!lua_isfunction(L, -1)) { lua_pop(L, 1); return; }
    lua_pushstring(L, ls(hookName));
    const status = lua_pcall(L, 1, 0, 0);
    if (status !== LUA_OK) {
      console.warn(`[mod] hook "${hookName}" error:`, lua_tojsstring(L, -1));
      lua_pop(L, 1);
    }
  }

  // ---------------------------------------------------------------------------
  // Load active mods (before user script — mods register hooks, don't run init)
  for (const { name, src: modSrc } of modSources) {
    try {
      execLua(modSrc, `mod:${name}`);
    } catch (e) {
      console.error(`[mod] failed to load "${name}":`, e.message);
    }
  }

  // ---------------------------------------------------------------------------
  // Parse and execute the user script (defines init, redraw, key, enc globals)

  callHook("script_pre_init");
  execLua(luaSrc, "user-script");

  // Call init() then fire post_init so mods can hook in after the script is ready
  callGlobal("init");
  callHook("script_post_init");
  ...
  return {
    ...
    cleanup() {
      ...
      callHook("script_pre_cleanup");
      clock.cleanup();
      midi.cleanup();
      softcut.reset();
      engineBridge.cleanup();
