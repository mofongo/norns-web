...
// Ports the norns display API to HTML Canvas 2D.
...
// Backing store dimensions (canvas pixels, not CSS pixels).
// Set during init() based on display size × devicePixelRatio.
let _backW = WIDTH;
let _backH = HEIGHT;
...
const screen = {
  ...
  // Uses the HiDPI technique: the canvas backing store is sized to
  // cssDisplaySize × devicePixelRatio, and the drawing context is
  // pre-scaled so all script coordinates remain in [0,WIDTH]×[0,HEIGHT].
  // This gives crisp text and shapes on Retina / high-DPI screens.
  init(canvas) {
    visibleCanvas = canvas;

    // CSS display size — read from the element's layout dimensions.
    // The element must already have a CSS width/height applied.
    const dpr  = window.devicePixelRatio || 1;
    const cssW = canvas.clientWidth  || WIDTH * 2;
    const cssH = canvas.clientHeight || HEIGHT * 2;

    // Backing store: one physical pixel per device pixel
    _backW = Math.round(cssW * dpr);
    _backH = Math.round(cssH * dpr);

    visibleCanvas.width  = _backW;
    visibleCanvas.height = _backH;
    visibleCtx = visibleCanvas.getContext("2d");

    // Offscreen drawing buffer (same backing size)
    offCanvas        = document.createElement("canvas");
    offCanvas.width  = _backW;
    offCanvas.height = _backH;
    ctx              = offCanvas.getContext("2d");

    // Scale: one unit in drawing space = (_backW/WIDTH) physical pixels.
    // All script calls (move, line, rect, text…) use [0,WIDTH]×[0,HEIGHT].
    const scaleX = _backW / WIDTH;
    const scaleY = _backH / HEIGHT;
    ctx.scale(scaleX, scaleY);

    // Defaults
    ctx.imageSmoothingEnabled = true;   // smooth shapes & text at device res
  ...
  clear() {
    ctx.save();
    ctx.setTransform(1, 0, 0, 1, 0, 0);  // bypass scale to fill backing store
    ctx.fillStyle = "rgb(0,0,0)";
    ctx.fillRect(0, 0, _backW, _backH);  // fill full device-resolution buffer
    ctx.restore();                        // restores the scale transform
    ctx.beginPath();
    _curX = 0;
    _curY = 0;
  },
