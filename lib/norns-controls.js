...
const KEY_DEFAULTS  = ['z', 'x', 'c']; // QWERTY keys for K1, K2, K3
...
export function buildNornsControls({ loader, onLog = () => {} }) {
  ...
  function buildKeys(container) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      // Label + keyboard shortcut hint (hidden when MIDI is assigned)
      btn.innerHTML = `K${n}<span class="nc-key-shortcut" id="nc-key-hint-${n}">${KEY_DEFAULTS[i].toUpperCase()}</span>`;
  ...
  // The hint (key letter) is visible when no MIDI note is assigned.
  // When MIDI is assigned, the keyboard mapping for that button is disabled
  // and the hint is hidden.

  function _updateKeyHint(idx) {
    const hint = document.getElementById(`nc-key-hint-${idx + 1}`);
    const btn  = document.getElementById(`nc-key-${idx + 1}`);
    if (!hint || !btn) return;
    const hasMidi = map.keys[idx] !== null;
    hint.style.display  = hasMidi ? 'none' : '';
    btn.title = hasMidi
      ? `K${idx + 1} — MIDI note ${map.keys[idx]}`
      : `K${idx + 1} — keyboard: ${KEY_DEFAULTS[idx].toUpperCase()}`;
  }

  // ── keyboard handling ─────────────────────────────────────────────────────
  // Default: Z → K1, X → K2, C → K3.
  // Disabled per-button when a MIDI note is assigned to that button.
  ...
  document.addEventListener('keydown', (e) => {
    ...
    const idx = KEY_DEFAULTS.indexOf(e.key.toLowerCase());
    if (idx < 0 || map.keys[idx] !== null) return; // MIDI assigned → skip
    if (_keysHeld.has(idx)) return;
    _keysHeld.add(idx);
    document.getElementById(`nc-key-${idx + 1}`)?.classList.add('pressed');
    loader.current?.key(idx + 1, 1);
  });
  ...
  document.addEventListener('keyup', (e) => {
    const idx = KEY_DEFAULTS.indexOf(e.key.toLowerCase());
    if (idx < 0 || map.keys[idx] !== null) return;
    if (!_keysHeld.has(idx)) return;
    _keysHeld.delete(idx);
    document.getElementById(`nc-key-${idx + 1}`)?.classList.remove('pressed');
    loader.current?.key(idx + 1, 0);
  });
  ...
  // Apply initial hint state from saved map
  for (let i = 0; i < 3; i++) _updateKeyHint(i);
